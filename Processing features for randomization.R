...
for (a in 1:length(call.timing.list)) {
  ...
  write.csv(AcousticIndicesDF,'data/AcousticIndicesDFPlayback.csv',row.names = F)
}
...
BirdNetFilesShort <- BirdNetFilesShort[which(RecorderID=='M1')]
BirdNetFiles <- BirdNetFiles[which(RecorderID=='M1')]
...
for(a in 1:length(BirdNetFiles)){ # 
  ...
  write.csv(BirdNetFeatures,'data/BirdNetFeaturesplayback.csv',row.names = F)
}
...
for(i in 1:length(TempfilesVGGish)){
  ...
  write.csv(VGGishDF,'data/VGGishDFPlayback.csv',row.names = F)
}
...
for(i in 1:length(TempfilesWav2Vec)){
  ...
  write.csv(wav2vecDF,'data/wav2vecDFPlayback.csv',row.names = F)
}
