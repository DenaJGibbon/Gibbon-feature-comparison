...
# Note for aplcluster used default q
# MFCC Randomization ------------------------------------------------------
...
for (a in 1:length(UniqueRecorderMFCC)) {
  ...
  for (b in 1:20) {
    ...
    print(TempMFCCRow)
    write.csv(RandomizationAccuracyMFCC, 'data/randomization_hdbscan/RandomizationAccuracyMFCC.csv')
  }
}
...
for (a in 1:length(UniqueRecorderBirdNET)) {
  ...
  for (b in 1:20) {
    ...
    write.csv(RandomizationAccuracyBirdNET, 'data/randomization_hdbscan/RandomizationAccuracyBirdNET.csv')
  }
}
...
for (a in 1:length(UniqueRecorderVGGish)) {
  ...
  for (b in 1:20) {
    ...
    print(TempVGGishRow)
    write.csv(RandomizationAccuracyVGGish, 'data/randomization_hdbscan/RandomizationAccuracyVGGish.csv')
  }
}
...
for (a in 1:length(UniqueRecorderwav2vec2)) {
  ...
  for (b in 1:20) {
    ...
    # Make predictions on the test data
    TestPredictions <- predict(ml.model.rf.wav2vec2.original, newdata = TestData[, -c(41,42,44)])
    ...
    AcousticSignals.umap <- umap::umap( TrainingData[, -c(41,42,44)],
                                        controlscale=TRUE,scale=3,n_neighbors=10)
...
for (a in 1:length(UniqueRecorderAcousticIndices)) {
  ...
  for (b in 1:20) {
    ...
    write.csv(RandomizationAccuracyAcousticIndices, 'data/randomization_hdbscan/RandomizationAccuracyAcousticIndices.csv')
  }
}
